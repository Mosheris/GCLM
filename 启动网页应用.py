...
# --- Model and Explainer Loading (最终修正: 使用 set_param 覆盖) ---
@st.cache_resource
def load_model():
    try:
        model = xgb.Booster()
        ...
        # 🎯 SHAP VALUE FIX (最终修正): 清理 base_score 字符串格式并强制覆盖 learner 参数

        # 1. 尝试从 attributes 中读取原始值
        ...
        if base_score_str:
            # 发现 base_score，但格式错误: '[7.065572E-2]'
        ...
        # 2. 关键步骤: 使用 set_param 强制覆盖 learner parameters 中的 base_score
        # SHAP 的 XGBTreeModelLoader 会读取这里的参数
...
def get_explainer(_model):
    # 保持原样，SHAP TreeExplainer 现在会读取 model.set_param() 中修正的 base_score
...
with col2:
    ...
    if predict_button:
        with st.spinner('Generating SHAP Force Plot...'):
            ...
            expected_value = explainer.expected_value # 从 Explainer 对象中获取
